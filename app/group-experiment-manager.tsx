...
function ExperimentConfigTable({ title, configs, onChange }: { title: string; configs: ExperimentDspConfig[]; onChange: (configs: ExperimentDspConfig[]) => void }) {
  ...
  return <div className="experiment-config-panel">
    <div className="experiment-config-toolbar"><span>{title}</span><button type="button" className="primary" onClick={addDsp}>＋ 添加PID</button></div>
    <div className="table-wrap experiment-config-table"><table><thead><tr><th>操作</th><th>DSP来源</th><th>PID</th><th>状态</th><th>底价</th></tr></thead><tbody>{configs.map((item) => <tr key={item.id}><td><button type="button" className="text-action" onClick={() => onChange(configs.filter((config) => config.id !== item.id))}>删除</button></td><td>{item.name}</td><td>{item.pids.join(" / ")}</td><td><button type="button" role="switch" aria-checked={item.enabled} className={`toggle ${item.enabled ? "on" : ""}`} aria-label={`${item.name}状态`} onClick={() => onChange(configs.map((config) => config.id === item.id ? { ...config, enabled: !config.enabled } : config))}><span /></button></td><td><span className="experiment-floor">¥<input aria-label={`${item.name}底价`} type="number" min="0" step="0.1" value={item.floor} onChange={(event) => onChange(configs.map((config) => config.id === item.id ? { ...config, floor: Math.max(0, Number(event.target.value)) } : config))} /></span></td></tr>)}{!configs.length && <tr><td colSpan={5}><div className="report-empty">暂无DSP来源，点击“添加PID”配置</div></td></tr>}</tbody></table></div>
  </div>;
}
