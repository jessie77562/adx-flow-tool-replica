...
function ExperimentConfigTable({ title, configs, pidOptions, onChange, allowBatchFloor = false, onNotify }: { title: string; configs: ExperimentDspConfig[]; pidOptions: ExperimentDspConfig[]; onChange: (configs: ExperimentDspConfig[]) => void; allowBatchFloor?: boolean; onNotify: (message: string) => void }) {
  const [selectedIds, setSelectedIds] = useState<number[]>([]);
  const [batchOpen, setBatchOpen] = useState(false);
  const [batchPrice, setBatchPrice] = useState("");
  const [batchError, setBatchError] = useState("");
  const [pidPickerOpen, setPidPickerOpen] = useState(false);
  const [pickedPidIds, setPickedPidIds] = useState<number[]>([]);
  const availablePidOptions = pidOptions.filter((option) => !configs.some((config) => config.id === option.id));
  ...
  const confirmPidSelection = () => {
    const picked = availablePidOptions.filter((option) => pickedPidIds.includes(option.id));
    onChange([...configs, ...cloneConfigs(picked)]);
    onNotify(`已选择 ${picked.length} 个 PID`);
    setPickedPidIds([]);
    setPidPickerOpen(false);
  };

  return <div className="experiment-config-panel">
    <div className="experiment-config-toolbar"><span>{title}</span><div>{allowBatchFloor && <button type="button" className="secondary" disabled={!selectedIds.length} onClick={() => setBatchOpen(true)}>批量修改底价{selectedIds.length ? `（${selectedIds.length}）` : ""}</button>}<button type="button" className="primary" onClick={() => setPidPickerOpen(true)}>＋ 选择PID</button></div></div>
    <div className="table-wrap experiment-config-table"><table><thead><tr>{allowBatchFloor && <th className="selection-cell"><input type="checkbox" aria-label="选择实验组全部DSP来源" checked={allSelected} onChange={toggleAll} /></th>}<th>操作</th><th>DSP来源</th><th>PID</th><th>状态</th><th>底价</th></tr></thead><tbody>{configs.map((item) => <tr key={item.id}>{allowBatchFloor && <td className="selection-cell"><input type="checkbox" aria-label={`选择实验组DSP来源 ${item.name}`} checked={selectedIds.includes(item.id)} onChange={() => toggleOne(item.id)} /></td>}<td><button type="button" className="text-action" onClick={() => { onChange(configs.filter((config) => config.id !== item.id)); setSelectedIds((current) => current.filter((id) => id !== item.id)); }}>删除</button></td><td>{item.name}</td><td>{item.pids.join(" / ")}</td><td><button type="button" role="switch" aria-checked={item.enabled} className={`toggle ${item.enabled ? "on" : ""}`} aria-label={`${item.name}状态`} onClick={() => onChange(configs.map((config) => config.id === item.id ? { ...config, enabled: !config.enabled } : config))}><span /></button></td><td><span className="experiment-floor">¥<input aria-label={`${item.name}底价`} type="number" min="0" step="0.1" value={item.floor} onChange={(event) => onChange(configs.map((config) => config.id === item.id ? { ...config, floor: Math.max(0, Number(event.target.value)) } : config))} /></span></td></tr>)}{!configs.length && <tr><td colSpan={allowBatchFloor ? 6 : 5}><div className="report-empty">暂无DSP来源，请从 PID 管理中选择已有 PID</div></td></tr>}</tbody></table></div>
    {pidPickerOpen && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setPidPickerOpen(false)}><section className="modal" role="dialog" aria-modal="true" aria-label="选择实验组PID"><header><h2>选择 PID</h2><button type="button" aria-label="关闭选择PID" onClick={() => setPidPickerOpen(false)}>×</button></header><div className="experiment-pid-picker">{availablePidOptions.map((option) => <label key={option.id} aria-label={`选择PID ${option.pids.join(" / ")}`}><input type="checkbox" checked={pickedPidIds.includes(option.id)} onChange={() => setPickedPidIds((current) => current.includes(option.id) ? current.filter((id) => id !== option.id) : [...current, option.id])} /><span><strong>{option.pids.join(" / ")}</strong><small>{option.name} · ¥{option.floor.toFixed(2)}</small></span></label>)}{!availablePidOptions.length && <div className="report-empty">当前分组暂无更多可选 PID</div>}</div><div className="modal-actions"><button type="button" className="secondary" onClick={() => setPidPickerOpen(false)}>取消</button><button type="button" className="primary" disabled={!pickedPidIds.length} onClick={confirmPidSelection}>确认选择</button></div></section></div>}
...
export default function GroupExperimentManager({ group, dsps, experiment, onBack, onChange, onNotify }: { group: GroupSummary; dsps: DspSummary[]; experiment?: GroupExperiment; onBack: () => void; onChange: (experiment: GroupExperiment) => void; onNotify: (message: string) => void }) {
  ...
  const pidOptions = seedConfigs(dsps);
  ...
  return <section className="panel experiment-page">
    ...
    {isCreate ? <section className="experiment-section"><h2>实验配置</h2><div className="experiment-tabs"><button type="button" className={activeConfig === "A" ? "active" : ""} onClick={() => setActiveConfig("A")}>对照组(A)</button><button type="button" className={activeConfig === "B" ? "active" : ""} onClick={() => setActiveConfig("B")}>实验组(B)</button></div>{activeConfig === "A" ? <ExperimentConfigTable title="A组已启用DSP来源" configs={aConfig} pidOptions={pidOptions} onChange={setAConfig} onNotify={onNotify} /> : copyAtoB ? <ExperimentConfigTable title="B组配置（同步A组）" configs={aConfig} pidOptions={pidOptions} onChange={setAConfig} allowBatchFloor onNotify={onNotify} /> : <ExperimentConfigTable title="B组已启用DSP来源" configs={bConfig} pidOptions={pidOptions} onChange={setBConfig} allowBatchFloor onNotify={onNotify} />}</section> : <>
