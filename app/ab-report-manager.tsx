...
export default function AbReportManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const exportDetail = () => {
    ...
    const exportRange = timeDimension === "day" ? `${dateRange.startDate.replaceAll("-", "")}_${dateRange.endDate.replaceAll("-", "")}` : `${hourlyDate.replaceAll("-", "")}_分时`;
    downloadCsv(`AB测试明细_${selectedMetric.label}_${exportRange}.csv`, [timeDimension === "day" ? "日期" : "时间", "A 对照组", "B 测试组", "对比涨幅"], detailRows.map((row) => [row.date, formatMetric(row.a[metric], selectedMetric.type), formatMetric(row.b[metric], selectedMetric.type), formatLift(metricLift(row.a[metric], row.b[metric]))]), onNotify);
  };
  ...
  return <section className="panel ab-report-panel">
    ...
    <section className="ab-report-card"><header className="ab-report-section-heading"><h2>数据明细 - {selectedMetric.label}<small>（{timeDimension === "day" ? "日维度" : "分时"}）</small></h2><button type="button" className="secondary" onClick={exportDetail}>导出</button></header><div className="table-wrap ab-detail-table"><table><thead><tr><th>{timeDimension === "day" ? "日期" : "时间"}</th><th>A 对照组</th><th>B 测试组</th><th>对比涨幅</th></tr></thead><tbody>{detailRows.map((row) => { const lift = metricLift(row.a[metric], row.b[metric]); return <tr key={row.date}><td>{row.date}</td><td>{formatMetric(row.a[metric], selectedMetric.type)}</td><td>{formatMetric(row.b[metric], selectedMetric.type)}</td><td className={lift === null || Math.abs(lift) < 0.00005 ? "flat" : lift > 0 ? "positive" : "negative"}>{formatLift(lift)}</td></tr>; })}{!detailRows.length && <tr><td colSpan={4}><div className="report-empty">暂无数据</div></td></tr>}</tbody></table></div></section>
