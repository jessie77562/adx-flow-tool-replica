...
function displayExperimentEnd(experiment: AbExperiment, dateOnly = false): string {
  if (experiment.status === "ongoing") return "至今";
  return dateOnly ? experiment.endAt.slice(0, 10) : experiment.endAt.replace("T", " ");
}

function displayStatisticsEnd(experiment: AbExperiment): string {
  return experiment.status === "ongoing" ? "2026-09-21" : experiment.endAt.slice(0, 10);
}
...
export default function AbReportManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const openExperimentDetail = (experimentId: number) => {
    ...
    setDetailExperimentId(String(experimentId));
  };
  ...
  const exportDetail = () => {
    ...
    const exportMetric = (row: (typeof detailRows)[number], group: "a" | "b", item: MetricOption) => {
      const value = formatMetric(row[group][item.key], item.type);
      if (!showGroupComparison || group === "a") return value;
      return `${value}（${formatLift(metricLift(row.a[item.key], row.b[item.key]))}）`;
    };
    if (timeDimension === "day") {
      const rows = detailRows.flatMap((row) => (["a", "b"] as const).map((group) => [row.date, group === "a" ? "A 对照组" : "B 测试组", ...comparisonMetrics.map((item) => exportMetric(row, group, item))]));
  ...
  const comparedMetric = (aValue: number, bValue: number, type: MetricType) => {
    const lift = metricLift(aValue, bValue);
    const liftClass = lift === null || Math.abs(lift) < 0.00005 ? "flat" : lift > 0 ? "positive" : "negative";
    return <><span>{formatMetric(bValue, type)}</span>{showGroupComparison && <small className={`ab-inline-lift ${liftClass}`}>{formatLift(lift)}</small>}</>;
  };
  ...
  if (!detailExperimentId) return <section className="panel ab-report-panel">
    ...
    <section className="ab-report-card ab-experiment-overview-card"><header className="ab-report-section-heading"><div className="ab-section-title"><h2>A/B 测试列表</h2><span>按实验创建时间倒序排列，共 {overviewExperiments.length} 个实验</span></div><div className="ab-list-filters"><label>分组名称<input className="ab-list-search" aria-label="分组名称筛选" list="ab-group-name-options" placeholder="全部" value={groupNameFilter} onChange={(event) => setGroupNameFilter(event.target.value)} /></label><datalist id="ab-group-name-options">{[...new Set(overviewSource.map((experiment) => experiment.optionLabel))].map((name) => <option key={name} value={name} />)}</datalist></div></header><div className="table-wrap ab-experiment-overview-table"><table><thead><tr><th>实验 ID</th><th>实验名称</th><th>实验开启时间</th><th>实验结束时间</th><th>实验状态</th><th>操作</th></tr></thead><tbody>{overviewExperiments.map((experiment) => <tr key={experiment.id}><td>{experiment.id}</td><td><strong>{experiment.testName}</strong><small>{experiment.optionLabel}</small></td><td>{experiment.startAt.replace("T", " ")}</td><td>{displayExperimentEnd(experiment)}</td><td><em className={`ab-list-status ${experiment.status}`}>{experiment.status === "ongoing" ? "进行中" : "已结束"}</em></td><td><button type="button" className="link-button" onClick={() => openExperimentDetail(experiment.id)}>查看详细实验数据</button></td></tr>)}{!overviewExperiments.length && <tr><td colSpan={6}><div className="report-empty">{groupNameFilter ? "未找到匹配的 A/B 测试" : "暂无 A/B 测试"}</div></td></tr>}</tbody></table></div></section>
  </section>;
