...
export default function AbReportManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const selectedExperiment = ongoingExperiments.find((experiment) => String(experiment.id) === selectedId) ?? initialExperiment;
  ...
  const changeExperiment = (nextId: string) => {
    const experiment = ongoingExperiments.find((item) => String(item.id) === nextId);
  ...
  const exportDetail = () => {
    ...
    downloadCsv(`AB测试明细_${selectedMetric.label}_${hourlyDate.replaceAll("-", "")}_分时.csv`, ["时间", "A 对照组", "B 测试组"], detailRows.map((row) => [row.date, formatMetric(row.a[metric], selectedMetric.type), formatMetric(row.b[metric], selectedMetric.type)]), onNotify);
  };
  ...
  return <section className="panel ab-report-panel">
    ...
    <section className="ab-report-card"><header className="ab-report-section-heading"><div className="ab-section-title"><h2>数据明细 - {timeDimension === "day" ? "所有指标" : selectedMetric.label}</h2><span>{timeDimension === "day" ? "按日期展示 A、B 两组全部指标" : "按小时展示 A、B 两组指标对比"}</span></div><div className="ab-detail-actions">{timeDimension === "day" ? <label>展示时间<div><input type="date" aria-label="明细开始日期" min={bounds.startDate} max={bounds.endDate} value={dateRange.startDate} onChange={(event) => updateDateRange({ startDate: event.target.value })} /><span>至</span><input type="date" aria-label="明细结束日期" min={bounds.startDate} max={bounds.endDate} value={dateRange.endDate} onChange={(event) => updateDateRange({ endDate: event.target.value })} /></div></label> : <label>展示日期<input type="date" aria-label="分时明细日期" min={bounds.startDate} max={bounds.endDate} value={hourlyDate} onChange={(event) => updateHourlyDate(event.target.value)} /></label>}<button type="button" className="secondary" onClick={exportDetail}>导出</button></div></header>{dateError && <div className="ab-date-error">{dateError}</div>}{timeDimension === "day" ? <div className="table-wrap ab-detail-table"><table><thead><tr><th className="ab-time-column">日期</th><th className="ab-group-column">组别</th>{comparisonMetrics.map((item) => <th key={item.key}><span className="report-metric-heading">{item.label}<button type="button" className="metric-help-button" aria-label={`查看${item.label}指标释义`} onClick={() => setDefinitionLabel(item.label)}>?</button></span></th>)}</tr></thead><tbody>{summaryA && summaryB && <><tr className="ab-detail-total"><td className="ab-time-column" rowSpan={2}>总计</td><td className="ab-group-column"><span className="ab-group-badge a">A</span>对照组</td>{comparisonMetrics.map((item) => <td key={item.key}>{formatMetric(summaryA[item.key], item.type)}</td>)}</tr><tr className="ab-detail-total"><td className="ab-group-column"><span className="ab-group-badge b">B</span>测试组</td>{comparisonMetrics.map((item) => <td key={item.key}>{formatMetric(summaryB[item.key], item.type)}</td>)}</tr></>}{detailRows.map((row) => <Fragment key={row.date}><tr><td className="ab-time-column" rowSpan={2}>{row.date}</td><td className="ab-group-column"><span className="ab-group-badge a">A</span>对照组</td>{comparisonMetrics.map((item) => <td key={item.key}>{formatMetric(row.a[item.key], item.type)}</td>)}</tr><tr><td className="ab-group-column"><span className="ab-group-badge b">B</span>测试组</td>{comparisonMetrics.map((item) => <td key={item.key}>{formatMetric(row.b[item.key], item.type)}</td>)}</tr></Fragment>)}{!detailRows.length && <tr><td colSpan={comparisonMetrics.length + 2}><div className="report-empty">暂无数据</div></td></tr>}</tbody></table></div> : <div className="table-wrap ab-hourly-detail-table"><table><thead><tr><th>时间</th><th>A 对照组</th><th>B 测试组</th></tr></thead><tbody>{detailRows.map((row) => <tr key={row.date}><td>{row.date}</td><td>{formatMetric(row.a[metric], selectedMetric.type)}</td><td>{formatMetric(row.b[metric], selectedMetric.type)}</td></tr>)}{!detailRows.length && <tr><td colSpan={3}><div className="report-empty">暂无数据</div></td></tr>}</tbody></table></div>}</section>
