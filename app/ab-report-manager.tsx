...
export default function AbReportManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const exportDetail = () => {
    ...
    downloadCsv(`AB测试明细_全指标_${exportRange}.csv`, [timeDimension === "day" ? "日期" : "时间", "组别", ...comparisonMetrics.map((item) => item.label)], rows, onNotify);
  };
